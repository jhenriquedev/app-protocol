import { type ApiResponse, BaseApiCase } from "../../../core/api.case";
...
interface TaskStore {
  ...
  reset?(): Promise<void>;
  update(
    updater: (current: Task[]) => Task[] | Promise<Task[]>,
  ): Promise<Task[]>;
}
...
export class TaskMoveApi extends BaseApiCase<TaskMoveInput, TaskMoveOutput> {
    ...
    return {
      ...
      handler: (request: {
        ...
        this.handler({
          ...
          targetStatus: request.body
            ?.targetStatus as TaskMoveInput["targetStatus"],
        }),
    };
  }
  ...
  public async test(): Promise<void> {
    ...
    if (
      invalidPersistedResult.success ||
