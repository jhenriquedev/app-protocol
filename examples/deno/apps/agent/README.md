...
- `tools/list` exposes a concise semantic summary derived from `prompt`,
  `discovery`, `context`, and policy
- MCP `resources/list` and `resources/read` expose the full projected semantic
  payload for each tool plus the host global system prompt
- `GET /catalog` mirrors the same registry-driven publication for human
