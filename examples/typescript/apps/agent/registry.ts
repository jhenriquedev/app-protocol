...
export function createRegistry(config: AgentConfig = {}): AgenticRegistry {
  ...
  const instantiateAgentic = (
    ...
    if (!AgenticSurface) {
      throw new Error(`Agentic surface not found for ${ref.domain}/${ref.caseName}`);
    }
  ...
  const buildCatalog = (ctx: AgenticContext): AgenticCatalogEntry[] =>
    listAgenticCases().map((ref) => {
      const surface = instantiateAgentic(ref, ctx);
      const definition = surface.definition();
      const requiresConfirmation = surface.requiresConfirmation();
      const entry: AgenticCatalogEntry = {
        ref,
        publishedName: "",
        definition,
        isMcpEnabled: surface.isMcpEnabled(),
  ...
  const resolveTool = (toolName: string, ctx: AgenticContext) =>
    buildCatalog(ctx).find(
      (entry) =>
        entry.publishedName === toolName || entry.definition.tool.name === toolName
    );
