...
function sendMcpHttpResponse(
  res: ServerResponse,
  response: AppMcpHttpResponse
): void {
...
export function bootstrap(config: AgentConfig = {}) {
  ...
  function createAgenticContext(
    ...
    const ctx: AgenticContext = {
      ...
      packages: registry._packages,
      mcp: {
        serverName: "angular-task-board-agent",
        version: APP_VERSION,
  ...
  function buildSystemPrompt(parent?: Partial<ParentExecutionContext>): string {
    ...
    const sections = [
      "You are operating angular-task-board-agent through the APP agent host.",
  ...
  async function handleRequest(
    ...
    if (await handleRemoteMcp(req, res)) {
      return;
    }
  ...
  async function startAgent(): Promise<Server> {
    const runtimeSummary = await validateAgenticRuntime();

    const server = createServer((req, res) => {
      void handleRequest(req, res).catch((error: unknown) => {
        logger.error("Unhandled agent scaffold error", {
          error: error instanceof Error ? error.message : "unknown",
        });
