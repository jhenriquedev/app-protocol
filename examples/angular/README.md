# Angular + Node Example — Task Board

Status: complete APP example with backend, portal, and agent hosts

This example demonstrates a complete APP setup in `examples/angular/` with an Angular portal, a Node backend, a dedicated `apps/agent` host, local JSON persistence, and three implemented Cases:
...
- frontend host: `apps/portal` using Angular
...
- `npm run dev:front` and `npm run start:front` use the Angular `production`
  serve configuration because that is the stable path for this APP host layout
...
curl -sS http://localhost:3000/tasks \
  ...
  -X POST \
  -d '{
    "title": "Ship the Angular APP example",
    "description": "Close the APP protocol review loop"
  }'
```
...
```bash
curl -sS http://localhost:3000/tasks/task_001/status \
  -H 'content-type: application/json' \
  -X PATCH \
  -d '{
    "targetStatus": "doing"
...
Inspect the published agentic catalog:

```bash
curl -sS http://localhost:3001/catalog
```

Execute a read-only tool through the agent host:
...
Execute a mutating tool through the agent host with explicit confirmation:
...
- each Case support artifact lives beside the surfaces as `<case>.us.md`
...
- portal visual composition happens in `apps/portal/src/root.component.ts`
- `src/root.component.ts` is not a second registry; it consumes the portal registry and composes UI Cases into the page shell
