...
process.stdout.write(
  "[dev] Running api on http://localhost:3000, front on http://localhost:3002, and agent HTTP on http://localhost:3001 (remote MCP at /mcp). Start MCP stdio separately with `npm run dev:agent:mcp` because stdio transport needs direct client ownership.\n"
);
