# Development Plan — Next.js + Node Task Board
...
- hosts:
  ...
  - `apps/portal` for Next.js UI execution
  - `apps/agent` for agentic catalog and tool execution
- framework runtime shell:
  - root `app/` for the Next.js App Router entrypoint
  - `apps/portal/app.ts` and `apps/portal/registry.ts` remain the canonical APP host bootstrap and registry
...
- copy or adapt the APP TypeScript `core/` contracts from the full reference example
- prepare `apps/backend`, `apps/portal`, and `apps/agent`
- scaffold the root `app/` runtime shell required by Next.js
...
- render the board page through Next.js App Router
- keep the APP host bootstrap in `apps/portal/app.ts`
...
- Next.js runtime entrypoint implemented in the root `app/` directory
...
- `npm run typecheck`, `npm run build:portal`, `npm run smoke`, and `npm run smoke:portal` pass
