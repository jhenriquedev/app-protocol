...
2. Given persisted tasks, `GET /tasks` returns success with the full collection ordered by `createdAt` descending.
3. Given corrupted persisted data with invalid task shape, the API returns a structured internal failure instead of silently masking corruption.

### UI

1. Given the board page loads, the UI requests the task list and renders three columns.
...
4. Structured API failures propagate to the host instead of collapsing into generic runtime errors.
