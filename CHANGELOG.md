...
- Root, installer, example, skill metadata, runtime headers, docs, and release alignment now advance to `1.1.5`
- The current working draft and the installable skill spec now point to `v1.1.5` as the latest released snapshot

### Fixed

- Re-cut the Claude Code compatibility patch on a correctly aligned release commit and tag after the broken `v1.1.4` tag publication
